...
class ChatEventoDetailsWidget extends StatefulWidget {
  const ChatEventoDetailsWidget({
    Key? key,
    this.chatRef,
    this.eventoAsoChat,
  }) : super(key: key);
  ...
  final EventosRecord? eventoAsoChat;

  @override
  _ChatEventoDetailsWidgetState createState() => _ChatEventoDetailsWidgetState();
}

class _ChatEventoDetailsWidgetState extends State<ChatEventoDetailsWidget> {
            ...
            children: [
                ...
                child: Row(
                  ...
                  children: [
                    ...
                    FlutterFlowIconButton(
                      ...
                      onPressed: () async {
                        context.pushNamed(
                          'DetalleEventoPertenezco',
                          queryParams: {
                            'chatPertenezco': serializeParam(
                              widget.chatRef,
                              ParamType.Document,
                            ),
                          }.withoutNulls,
                          extra: <String, dynamic>{
                            'chatPertenezco': widget.chatRef,
                          },
                        );
                      },
                    ),
              ...
              Container(
                ...
                child: TextField(
                  ...
                  onSubmitted: (value) {
                    sendChat(context, widget.chatRef, value);
                  },
                ),
              )
            ],
  ...
  void sendChat(context, chatref, message) {
    ...
    if(check(message)) {
      ...
      DatabaseReference ref = FirebaseDatabase.instance.ref().child('chats/' + chatref.evento.id);
      ref.push().set({
        'sender': uid,
        'senderName': userName,
        'message': message
